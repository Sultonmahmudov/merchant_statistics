...
with col1:
    st.image("https://upload.wikimedia.org/wikipedia/commons/a/a7/React-icon.svg", width=50)
with col2:
...
if uploaded_files:
    ...
    st.metric("Jami tushum (so'm)", f"{total_amount:,.0f}")
    st.metric("O‘rtacha tushum (so'm)", f"{avg_amount.mean():,.0f}")
    # --- BOX Bar Chart ---
